...
inputDir="/afs/cern.ch/user/m/mfiascar/workdir/Accelerator/Simulations/FCC/v10_fromBarbara/BetatronIR2"

#Output directory
#outputDir="/afs/cern.ch/user/m/mfiascar/workdir/Accelerator/Simulations/Outputs/" #used only if LXPLUS
LocalPWD="/home/mfiascar/Physics/Accelerator/Simulation/FCC/v10/BetatronIR2_fullSim" #used for local machine
...
beamLossPatt="${executeDir}/BeamLossPattern_forFCC"
...
##  ./get_all_TCTVA5_tracks.sh 
  ...
  mkdir run$index/
  cp  Coll_Scatter* collgaps.dat coll_summary.dat survival.dat LPI* impacts_all* impacts_real.dat first_imp_average.dat FirstImpacts.dat FLUKA_impacts.dat dist0.dat efficiency*.dat run$index/
...
if [ -d "run$index" ]; then
    ...
    bsub -q $queue -o /dev/null -e /dev/null -sp 100 -R "rusage[pool=50000]" SixTr--$index.job
